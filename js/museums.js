async function cargarMuseos() {
    const contenedor = document.getElementById('list-museums');

    try {
        // 1. Cargar el JSON en inglés
        const respuesta = await fetch('./assets/museums.json');
        ...
        museos.forEach(museo => {
            ...
            tarjeta.innerHTML = `
                ...
                <div class="grid grid-cols-2 gap-2 pt-2 border-t border-black/10 text-center">
                    <div>
                        <h4 class="font-bold text-xs text-black">Opening Hours</h4>
                        ${horariosHtml}
                    </div>
                    <div>
                        <h4 class="font-bold text-xs text-black">Admission Rates</h4>
                        ${costosHtml}
                    </div>
                </div>
            `;
        ...
        contenedor.innerHTML = `<p class="text-center text-xs text-red-400">Failed to load museum information.</p>`;
    }
}
